...
export function GameStage({ currentEvent }: GameStageProps) {
    ...
    return (
        <main className="flex-1 overflow-y-auto w-full flex items-center justify-center p-4 md:p-8 relative overflow-hidden">
            ...
            <div
                className={`pointer-events-none absolute inset-0 z-0 bg-gradient-to-t ${sev.overlay} transition-colors duration-1000`}
            />
                ...
                {showAlarm && (
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center text-center">
                            ...
                            <span
                                className={`text-xs md:text-sm font-bold ${sev.bannerText} uppercase tracking-widest mt-0.5`}
                            >
                                {sev.label[language]}
                                {" · "}
                                {language === "vi"
                                    ? `Mức độ ${currentEvent.severity}/3`
                                    : `Level ${currentEvent.severity}/3`}
                            </span>
                        </div>
            ...
            <div id="tutorial-event-area" className="max-w-5xl w-full flex flex-col md:flex-row items-center justify-center gap-6 md:gap-12 pb-10 mt-14 md:mt-4 relative z-10">
                ...
                >
                    {/* Bubble tail – Desktop */}
                    <div
                        className="hidden md:block absolute -left-8 top-1/2 -translate-y-1/2 w-0 h-0
